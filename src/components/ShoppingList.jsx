...
const ShoppingList = () => {
    ...
    return (
        <div className='sm:p-0 bg-white mt-4 text-black py-5 md:px-8 h-auto md:flex justify-between box-border'>
            <div className='p-4'>
                <h1 className='md:text-4xl text-blue-600 font-bold text-center text-2xl '>Shopping App 🛒</h1>
                <div className='md:w-96 w-full md:max-W-xs mt-4'>
            ...
            <div className='md:border-l-2 border-blue-600 p-4 md:w-3/5'>
                <ul className='min-h-48'>
                    {shoppingList.length > 0 ? shoppingList.map((item, index) => (
                        <li key={index} className={item.isChecked ? 'line-through flex justify-between border-b-2 border-blue-400 md:p-3 p-2' : 'flex justify-between border-b-2 border-blue-400 md:p-3 p-2'}>
                            ...
                            <span className='w-1/6 mr-3'>${item.price}</span>
                            <div>
                ...
                <div className='border shadow-lg bg-gray-300 mt-10 h-16 lg:py-4 lg:px-6 py-2 px-2 '>
                    <p className='flex justify-between text-center'><span className='mr-2'>Total: ${totalPrice}</span><span className='mr-2'>Spent: ${checkedPrice}</span><span>Balance: ${uncheckedPrice}</span></p>
                </div>
            </div>
        </div>
    )
